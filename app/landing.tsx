export default function Landing({ configured }: { configured: boolean }) {
  return <main className="landing-shell">
    ...
    <section className="landing-hero">
      <div className="landing-copy"><span className="eyebrow">PRIVATE SKIN JOURNAL</span><h1>用自己的方式<br/>追蹤每一次變化</h1><p>自由建立追蹤專案，每天拍攝或匯入任意張照片，再依實際拍攝時間回顧與比較。</p><a href="/auth/google" className="google-button"><span>G</span>使用 Google 帳號開始</a>{!configured && <div className="setup-notice">Google 登入服務正在等待管理員完成連線設定。</div>}<small className="login-privacy">我們不會取得或儲存你的 Gmail 密碼。每個帳號的照片與紀錄完全分開。</small></div>
    ...
    <section className="landing-features"><article><span>01</span><h2>自由建立追蹤專案</h2><p>專案名稱與每日照片數量都由你決定，人臉角度提示也可各自開關。</p></article><article><span>02</span><h2>每個帳號獨立</h2><p>登入身分由 Google 驗證，所有專案、照片與日誌皆由伺服器強制隔離。</p></article><article><span>03</span><h2>任選照片回顧比較</h2><p>依實際拍攝日期時間查看、排序，並可跨專案任選兩張照片並排比較。</p></article></section>
