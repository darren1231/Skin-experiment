...
test("projects accept custom names and explicit face-guidance checkbox only", async () => {
  const [schema, route, dashboard] = await Promise.all([
    read("db/schema.ts"), read("app/api/projects/route.ts"), read("app/dashboard.tsx"),
  ]);
  assert.match(schema, /trackingProjects/);
  assert.match(schema, /enableFaceAngleGuidance/);
  assert.match(route, /const name = String\(body\.name/);
  assert.match(route, /enableFaceAngleGuidance:\s*body\.enableFaceAngleGuidance === true/);
...
test("each photo is independent with capture and upload timestamps", async () => {
  const [schema, route] = await Promise.all([read("db/schema.ts"), read("app/api/project-photos/route.ts")]);
  for (const field of ["projectId", "storageKey", "capturedDate", "capturedTime", "capturedAt", "uploadedAt", "photoSource", "note"]) {
    assert.match(schema, new RegExp(`${field}:`));
  }
  assert.match(route, /capturedAt = `\$\{capturedDate\}T\$\{capturedTime\}/);
  assert.match(route, /uploadedAt:\s*now/);
  assert.match(route, /SOURCES = \["camera", "album", "historical_import"\]/);
  assert.doesNotMatch(schema, /uniqueIndex\([^)]*\)\.on\(table\.projectId,\s*table\.capturedDate/);
});
...
test("library filters and sorts by actual capture metadata", async () => {
  ...
  assert.match(route, /orderBy\(desc\(projectPhotos\.capturedDate\), desc\(projectPhotos\.capturedTime\)/);
});
...
test("migration is additive and maps legacy photos without guessing capture time", async () => {
  const migration = await read("drizzle/0002_sticky_multiple_man.sql");
  assert.match(migration, /CREATE TABLE `tracking_projects`/);
  assert.match(migration, /CREATE TABLE `project_photos`/);
  assert.match(migration, /'臉部肌膚追蹤'/);
  assert.match(migration, /'legacy-face-' \|\| `users`\.`id`/);
  assert.match(migration, /`face_photos`\.`object_key`/);
  assert.match(migration, /'legacy_date_only'/);
  assert.match(migration, /`captured_time`, `captured_at`/);
...
test("all project, photo and binary APIs enforce verified user ownership", async () => {
  ...
  assert.match(projects, /eq\(trackingProjects\.userId, user\.id\)/);
  assert.match(photos, /eq\(projectPhotos\.userId, user\.id\)/);
  assert.match(binary, /eq\(projectPhotos\.userId, user\.id\)/);
});
